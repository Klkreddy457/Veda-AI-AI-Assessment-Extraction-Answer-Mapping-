...
class PyMuPDFVisionProvider(VisionProvider):
    ...
    # Match question patterns e.g. "1. What is...", "4(a) Define...", "4b) Give..."
    QUESTION_PATTERN = re.compile(
        r"^(?:Q|q)?\s*(\d+[\.\:\-\)]?|\d+\s*[\(\[]?[a-zA-Z][\)\]\.]?)\s+(.+)$"
    )

    # Match answer headers e.g. "Ans 4b)", "1)", "7)", "2)", "4a)", "9)", "99)", "5)", "8)", "10)", "6)"
    ...
    def extract_question_paper(self, file_bytes: bytes, filename: str) -> List[Dict[str, Any]]:
            ...
            for page_idx, page in enumerate(doc, start=1):
                ...
                for block in blocks:
                    ...
                    for line in lines:
                        ...
                        # Check if line matches a numbered question
        ...
        # If PDF text extraction yielded results, return them!
        if extracted_questions:
            return extracted_questions

        # Fallback to fixture data if document has no embedded text layer
    ...
    def extract_answer_sheet(self, file_bytes: bytes, filename: str) -> List[Dict[str, Any]]:
        ...
        # High quality fallback fixture dataset matching Computer Science Test answer sheet
